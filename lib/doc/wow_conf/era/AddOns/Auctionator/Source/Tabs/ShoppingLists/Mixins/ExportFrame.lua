...
function AuctionatorListExportFrameMixin:InitializeCheckBoxes()
  self.checkBoxPool = {}
  self.listCount = #Auctionator.ShoppingLists.Lists

  -- Create enough frames for current number of lists
  for _, _ in ipairs(Auctionator.ShoppingLists.Lists) do
    self:AddToPool()
  end
...
function AuctionatorListExportFrameMixin:OnExportClicked()
  ...
  for _, checkbox in ipairs(self.checkBoxPool) do
    if checkbox:IsVisible() and checkbox:GetChecked() then
      exportString = exportString .. Auctionator.ShoppingLists.GetBatchExportString(checkbox:GetText()) .. "\n"
    end
  end
  ...
  -- else
    ...
    -- for _, checkbox in ipairs(self.checkBoxPool) do
    --   if checkbox:IsVisible() and checkbox:GetChecked() then
    --     C_ChatInfo.SendAddonMessage( "Auctionator", Auctionator.ShoppingLists.GetBatchExportString(checkbox:GetText()), "WHISPER", self.Recipient:GetText())
    --   end
    -- end
